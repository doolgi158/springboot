...
@Entity
// 엔티티와 매핑할 테이블을 지정 - 생략하면 매핑할 엔티티 이름을 테이블명으로 사용. name 속성을 이용해서 테이블명 지정.
...
public class Board {
    ...
    //엔티티가 생성(@CreationTimespamp)되거나 업데이트(@UpdateTimestamp)되는 시점의 날짜 데이터를 기록하는 설정
    @CreationTimestamp
    ...
    // 객체 타입을 사용하는 이유는 null을 리턴할 수 있기 때문에.
}
